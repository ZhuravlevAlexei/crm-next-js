...
export interface PromotionInerface {
  id: string;
  title: string;
  description: string;
  discount: number;
...
export const getPromotions = async (
  ...
  return sendRequest<PromotionInerface[]>(
...
export const createPromotion = async (
  data: Omit<PromotionInerface, 'id'>,
  init?: RequestInit,
) => {
  return sendRequest<PromotionInerface>(buildUrl('promotions'), {
    method: 'POST',
    body: JSON.stringify(data),
    headers: {
      ...(init && init.headers),
